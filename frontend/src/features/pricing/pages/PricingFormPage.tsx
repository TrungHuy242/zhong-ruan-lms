/**
 * PricingFormPage — trang tạo/sửa bảng giá (Admin).
 *
 * Hallmark stamp:
 ...
 *
 * Routing:
 *   /pricing-plans/new        → create mode
 *   /pricing-plans/:id/edit   → edit mode
 *
 * Đặc thù Pricing:
 ...
 *
 * Thay thế PricingFormModal — URL bookmarkable, Back tự nhiên, full-page layout.
 */
...
import { usePricingFeaturedCount } from "../hooks/usePricingFeaturedCount";
import styles from "./PricingFormPage.module.css";
...
export function PricingFormPage() {
  ...
  // ===== Track featured count để cảnh báo =====
  const featuredCount = usePricingFeaturedCount(isEdit ? id ?? null : null);
  ...
  useEffect(() => {
    ...
    getPricingPlan(id)
      .then((p) => {
        ...
        setPriceInput(formatPriceInput(String(p.price)));
        setOriginalPriceInput(
          p.originalPrice ? formatPriceInput(String(p.originalPrice)) : ""
        );
        setUnit(p.unit);
        setDescription(p.description ?? "");
        setBenefitsInput(p.benefits ?? []);
        setBenefitInput("");
        setLinkedCourseSlug(p.linkedCourseSlug ?? "");
        setIsFeatured(p.isFeatured);
        setIsPublished(p.isPublished);
        setDisplayOrder(String(p.displayOrder ?? 0));
        setLoading(false);
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    document.title = isEdit
      ? `Sửa bảng giá — Zhong Ruan LMS`
      : `Thêm bảng giá — Zhong Ruan LMS`;
  }, [isEdit]);
  ...
  useEffect(() => {
    if (isFeatured) {
      const alreadyFeatured = isEdit
        ? featuredCount // đang sửa, không tính plan này
        : featuredCount + 1; // đang tạo mới
  ...
  }, [isFeatured, featuredCount, isEdit]);
  ...
  function handleOriginalPriceChange(e: ChangeEvent<HTMLInputElement>) {
    setOriginalPriceInput(formatPriceInput(e.target.value));
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (isEdit && currentPlan) {
        await updatePricingPlan(currentPlan.id, payload);
        toast.success("Đã cập nhật bảng giá");
        window.dispatchEvent(new CustomEvent("lms:pricing-updated"));
      } else {
  ...
  if (notFound) {
    return (
      <div className={styles.page}>
        <Alert variant="error">
          <strong>Không tìm thấy bảng giá.</strong> Có thể đã bị xoá hoặc URL không hợp lệ.
        </Alert>
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        ...
        description={
          isEdit
            ? `Cập nhật thông tin cho ${currentPlan?.name ?? ""}`
            : "Tạo gói giá mới cho trang Bảng giá public"
        }
        breadcrumb={[
      ...
      <form onSubmit={handleSubmit} noValidate className={styles.form}>
        ...
        {/* Cảnh báo nổi bật */}
        {featuredWarning ? <Alert variant="warning">{featuredWarning}</Alert> : null}
